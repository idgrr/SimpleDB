...
//Methods to check the status of the current token
   ...
   public boolean matchAggregateFn() {
	   Collection<String> aggregateList = Arrays.asList("sum", "avg", "count", "min", "max");
	   for (String w: aggregateList) {
	      if( tok.ttype == StreamTokenizer.TT_WORD && tok.sval.equals(w)) {
	    	  return true;
	      	}
	      }
	   return false;}
...
//Methods to "eat" the current token
   ...
   private void initKeywords() {
      keywords = Arrays.asList("select", "from", "where", "and",
                               ...
                               "using", "hash", "btree", "order", "by", "asc", "desc", "distinct");
   }
}